...
function addSuraCells() {
  ...
  for (var cellIndex = 1; cellIndex <= 114; cellIndex++) {
    ...
    var daysElapsedText =
      daysElapsed == 0 || daysElapsed > 1000 ? "" : daysElapsed + " Days";
      ...
      if (snapshot != null) {
        snapshot.forEach(function(childSnapshot) {
          ...
          switch (transactionRecord.op) {
            ...
            case "refresh":
              if (
                surasHistory[suraIndex].history.indexOf(
  ...
  firebase.auth().onAuthStateChanged(function(user) {
    ...
    if (user) {
      ...
      update_timestamp_ref.on("value", function(snapshot) {
        ...
        timeStampTriggerTimerRef = setTimeout(
          onTimeStampUpdated,
          isFirstLoad == 1 ? 0 : 5000
        );
        isFirstLoad = 0;
      });
    } else {
