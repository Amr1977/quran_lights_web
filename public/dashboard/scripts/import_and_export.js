...
function exportJSON() {
  let filename = "quran_lights_history.json";

  let blob = new Blob([export_history_to_json()], { type: "text/csv;charset=utf-8;" });
...
function importJSON() {
  ...
  file_reader.onload = function (e) {
    let result = is_json_string(e.target.result);
    if (result[0]) {
...
function merge_imported_suras_history(history){
  ...
  for (let suraIndex in history) {
    ...
    if (
      ...
      let new_records = history[suraIndex].history.filter( x => !surasHistory[suraIndex].history.includes(x));
  ...
  for (let suraIndex in surasHistory.keys) {
    surasHistory[suraIndex].history.sort(sortNumber);
  }
